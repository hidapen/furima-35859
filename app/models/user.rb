class User < ApplicationRecord
  ...
  with_options presence: true do
    ...
    validates :first_name_furigana, format: {with: /\A[ァ-ヶー]+\z/, message: "は全角（カタカナ）で入力してください"}
    validates :last_name_furigana, format: {with: /\A[ァ-ヶー]+\z/, message: "は全角（カタカナ）で入力してください"}
    validates :birthday
  end
end
